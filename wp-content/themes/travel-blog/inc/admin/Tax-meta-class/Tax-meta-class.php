...
if ( !class_exists( 'Tax_Meta_Class' ) ) :
	...
	class Tax_Meta_Class {
		...
		public function show_field_icon( $field, $meta ) {
			...
			<input alt="#TB_inline?height=400&width=500&inlineId=iconbox-popup" title="<?php esc_attr_e( 'Click to browse icon', 'travel-blog' ) ?>" class="thickbox button-secondary submit-add-to-menu" type="button" value="<?php esc_attr_e( 'Browse Icon', 'travel-blog' ) ?>" onclick="on_open_icon_browser('edit-<?php echo esc_attr( $field['id'] ); ?>');" />
		...
		public function show_field_image( $field, $meta ) {
			...
			if ( $has_image ) {
				echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='" . esc_html__( 'Remove Image', 'travel-blog' ) . "' type='button'/>";
			} else {
				echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='" . esc_html__( 'Upload Image', 'travel-blog' ) . "' type='button'/>";
			}
			echo '</div>';
			$this->show_field_end( $field, $meta );
		}
