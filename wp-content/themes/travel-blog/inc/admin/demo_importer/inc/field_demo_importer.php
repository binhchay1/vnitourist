	...
	class ReduxFramework_demo_importer {
		...
		function __construct( $field = array(), $value = '', $parent ) {
			...
			if ( !empty( $class->demo_data_dir ) ) {
				...
				$this->demo_data_url = TRAVEL_BLOG_THEME_URI . '/inc/admin/demo_importer/demo-data/';
			}

			if ( empty( $this->extension_dir ) ) {
				$this->extension_dir = trailingslashit( str_replace( '\\', '/', get_template_directory() . '/inc/admin/demo_importer/inc' ) );
				$this->extension_url = TRAVEL_BLOG_THEME_URI . '/inc/admin/demo_importer/inc';
			}
		}
			...
			if ( !empty( $this->field['demo_demo_imports'] ) ) {
				...
				foreach ( $this->field['demo_demo_imports'] as $section => $imports ) {
					...
					if ( !array_key_exists( 'imported', $imports ) ) {
						...
						$imported       = false;
						$import_message = esc_html__( 'Import Demo', 'travel-blog' );
					} else {
						$imported       = true;
						$extra_class    = 'active imported';
						$import_message = esc_html__( 'Demo Imported', 'travel-blog' );
					}
					...
					if ( false == $imported ) {
						echo '<div class="demo-importer-buttons"><span class="spinner">' . esc_html__( 'Please Wait...', 'travel-blog' ) . '</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', 'travel-blog' ) . '</span></div>';
					} else {
						echo '<div class="demo-importer-buttons button-secondary importer-button">' . esc_html__( 'Imported', 'travel-blog' ) . '</div>';
						echo '<span class="spinner">' . esc_html__( 'Please Wait...', 'travel-blog' ) . '</span>';
						echo '<div id="demo-importer-reimport" class="demo-importer-buttons button-primary import-demo-data importer-button">' . esc_html__( 'Re-Import', 'travel-blog' ) . '</div>';
					}
					echo '</div>';
					echo '</div>';
				...
				echo "<h5>" . esc_html__( 'No Demo Data Provided', 'travel-blog' ) . "</h5>";
			}
