	...
	class ReduxFramework_extension_demo_importer {
		...
		public function __construct( $parent ) {
			...
			if ( empty( $this->extension_dir ) ) {
				$this->extension_dir = trailingslashit( str_replace( '\\', '/', get_template_directory() . '/inc/admin/demo_importer' ) );
			...
			include TRAVEL_BLOG_THEME_DIR . '/inc/admin/demo_importer/inc/class-demo-importer-progress.php';
		...
		public function ajax_importer() {
			...
			if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->data_import_files ) ) {
				...
				if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
					include TRAVEL_BLOG_THEME_DIR . '/inc/admin/demo_importer/inc/init-installer.php';
					$installer = new Radium_Theme_Demo_Data_Importer( $this, $this->parent );
				} else {
					echo esc_html__( "Demo Already Imported", 'travel-blog' );
				}
		...
		public function overload_field_path( $field ) {
			return get_template_directory() . '/inc/admin/demo_importer/inc/field_demo_importer.php';
		}
		...
		function add_importer_section() {
			...
			$demo_importer_label = trim( esc_html( apply_filters( 'demo_importer_label', __( 'Demo Importer', 'travel-blog' ) ) ) );

			$demo_importer_label = ( !empty( $demo_importer_label ) ) ? $demo_importer_label : __( 'Demo Importer', 'travel-blog' );

			$this->parent->sections[] = array(
				'id'     => 'demo_importer_section',
				'title'  => $demo_importer_label,
				'desc'   => '<p class="description">' . apply_filters( 'demo_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'travel-blog' ) ) . '</p>',
