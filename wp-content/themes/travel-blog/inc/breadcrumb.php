 ...
 *
 */
function travel_blog_get_category_parents( $id, $link = false, $separator = '', $nicename = false, $visited = array() ) {
	$chain  = '';
 ...
 */
function travel_blog_breadcrumbs() {
	global $wp_query, $post;
	$show_on   = get_option( 'show_on_front' );
	$page_id   = get_option( 'page_for_posts' );
	$delimiter = "";
	echo '<ul class="phys-breadcrumb">';
	echo '<li><a href="' . esc_url( home_url( '/' ) ) . '" class="home">' . esc_html__( "Home", 'travel-blog' ) . '</a>' . $delimiter . '</li>';
	if ( is_category() ) {
		...
		echo '<li>' . esc_html__( "Search Result", "travel-blog" ) . $delimiter . '</li>';
	} elseif ( is_404() ) {
		echo '<li>' . esc_html__( "404 Not Found", "travel-blog" ) . $delimiter . '</li>';
	} elseif ( is_single( $post ) ) {
		if ( get_post_type() == 'post' ) {
			$category    = get_the_category();
			$category_id = get_cat_ID( $category[0]->cat_name );
			echo '<li>' . travel_blog_get_category_parents( $category_id, true, $delimiter ) . '</li>';
